...
({
	...
	//datagrid(list view)
	LVN_GETDISPINFO,
	NM_CLICK,//parent window notified when an row(item) is single clicked
...
LRESULT CALLBACK Window::MessageRouter(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (wnd)
	{
		...
		case WM_NOTIFY:
		{
			...
			if (lpnmhdr->code & (TCN_SELCHANGING | LVN_BEGINLABELEDIT | LVN_ENDLABELEDIT | LVN_GETINFOTIP | LVN_ITEMCHANGING))
			{
...
void Window::AddHandler(unsigned int message, MessageHandler handler)
{
	if (window_message_map.find(message) == window_message_map.end())
		window_message_map.insert(std::pair<long, MessageHandler>(message, handler));
	else
