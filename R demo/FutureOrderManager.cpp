...
static DWORD WINAPI CancelOrderThread(LPVOID lpParam)
{
	...
	if (it != order_operation_credential->invest_account->_order_list.end())
	{
		...
		{
			local_future_order_manager->order->Cancel(order_operation_credential->broker_info, order_operation_credential->invest_account, order_operation_credential->order_credential);
		}
	}
...
void FutureOrderManager::RegisterOpenOrder(BrokerInfo* broker_info, InvestAccountSync* invest_account, OrderCredential* _oc)
{
	...
	order_operation_credential->broker_info = broker_info;
