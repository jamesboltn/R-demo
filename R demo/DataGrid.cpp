...
DataGrid::DataGrid(std::vector<wchar_t*>* column_headers, int x_pos, int y_pos, int width, int height, UnitType type, char* _unique_name)
{
	//we're going to assume the following members are declared in Window as protected
	//lpWindowName = L"";
	lpClassName = WC_LISTVIEW;
	dwStyle = WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_EDITLABELS | LVS_ALIGNTOP;
	y = y_pos;
	x = x_pos;
	Height = height;
	Width = width;
	...
	for (auto it = column_headers->begin(); it != column_headers->end(); ++it)
	{
		...
		col_name_index_map.insert(std::make_pair(column_header, column_count++));
	}

	
}
void DataGrid::SetViewStyle(long new_view_style)
{
...
void DataGrid::InsertCell(int row_index, int col_index, const wchar_t* text)
{
...
void DataGrid::InsertCellA(int row_index, int col_index, const char* text)
{
...
void DataGrid::InsertCellA(int row_index, const std::string &col_name, const char* text)
{
...
void DataGrid::DeleteAll()
{
	ListView_DeleteAllItems(hwnd);
}
void DataGrid::DeleteRow(int row_index)
{
...
int DataGrid::GetSelectedRowIndex()
{
	return ::SendMessage(hwnd, LVM_GETNEXTITEM, WPARAM(-1), LVNI_FOCUSED);
}
void DataGrid::GetCellText(int row_index, int col_index, wchar_t* buffer, int length)
{
	LVITEM LvItem;
...
void DataGrid::GetCellTextA(int row_index, int col_index, char* buffer, int length)
{
	LVITEMA LvItem;
	ZeroMemory(&LvItem, sizeof(LvItem));
	LvItem.mask = LVIF_TEXT;
...
void DataGrid::SetRowHeight(int height)
{
...
void DataGrid::SetCheckBoxState(unsigned int index, bool state)
{
#pragma warning(push)
...
bool DataGrid::GetCheckBoxState(int index)
{
...
void DataGrid::AddGroupsCollapsible(std::vector<wchar_t*> group_headers, Align align)
{
...
void DataGrid::AddGroupCollapsible(wchar_t* group_header, int index, Align align)
{
	ListView_EnableGroupView(hwnd, TRUE);
...
void DataGrid::AddGroupsUncollapsible(std::vector<wchar_t*> group_headers, Align align)
{
	ListView_EnableGroupView(hwnd, TRUE);
...
void DataGrid::AddGroupUncollapsible(wchar_t* group_header, int index, Align align)
{
	ListView_EnableGroupView(hwnd, TRUE);
...
void DataGrid::MoveRowToGroup(int group_index, int row_index)
{
	LVITEMA Item = { 0 };
