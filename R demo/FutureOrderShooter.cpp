...
void FutureOrderShooter::ConnectOrderServer()
{
	...
	for (auto it = Global::memory.order_broker_list.begin(); it != Global::memory.order_broker_list.end(); ++it)
	{
...
#pragma region check for future accounts with no order server supplied
		if (it->order_front.first.empty())
		{
			...
			{
				strcpy_s(investor_id, ARRAYCOUNT(investor_id), it1->second.investor_id.c_str());
				folder_path += investor_id;
				CreateDirectoryA(folder_path.c_str(), NULL);

				instance_log_path = "./Log/" + it->broker_name + "/" + it1->second.investor_id + "/";
				it1->second.orderapi_spi_pair = std::make_pair(nullptr, nullptr);

				msg = "you haven't specified any CTP future order server of " + Global::memory.order_broker_list[index].broker_name;

				//show error message on window
				(*handler)(msg.c_str(), Global::memory.order_broker_list[index++].broker_name.c_str(), instance_log_path.c_str());
			}
...
#pragma region instantiate api_spi pairs
			folder_path_a += it1->second.investor_id;
			folder_path_a += "//";
			...
			orderspi_instance = new FutureOrderProcessor(this, &(*it), &(it1->second), index++);
			...
			it1->second.orderapi_spi_pair = std::make_pair(orderapi_instance, orderspi_instance);
#pragma endregion
		}
#pragma endregion 
	}
...
void FutureOrderShooter::Shoot(char* contract, double price, Direction direction, OpenFlag open_flag, int volume, OrderBroker* order_broker, InvestAccountSync* invest_account)
{
...
void FutureOrderShooter::Cancel(OrderBroker* order_broker, InvestAccountSync* invest_account, OrderCredential* oc)
{
