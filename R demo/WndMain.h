...
{
private:
	WndChooseAccount* wnd_choose_account;
...
public:
	...
	bool is_contract_dic_retrieved;//是否已经更新过合约字典
	...
	void CheckBrokerConnection(BrokerInfoList* broker_info_list);
	static bool ShowDatabaseErrorMessage(const std::string &msg);//if returns true means there is error in database process
	static bool ShowDatabaseErrorMessage(const std::string &msg, size_t index);
	static void FutureOrderHandler(const char* msg, size_t index, const char* log_path = NULL);//this handler shows error message generated by future order or sets log path of each EditWithLog control to the path supplied with each future order instance 
