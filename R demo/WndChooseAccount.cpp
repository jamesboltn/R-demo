...
#pragma region enable login button after check
	...
	for (int i = 0; i < wnd_choose_account->datagrid_account_list.GetRowCount(); ++i)
	{
...
void WndChooseAccount::InitComponents()
{
	...
	if (!Global::memory.broker_info_list.empty())
	{
		...
		for (auto it = Global::memory.broker_info_list.begin(); it != Global::memory.broker_info_list.end(); ++it)
		{
			...
			{
				datagrid_account_list.InsertCellA(account_count, 0, &(it->broker_name + it1->investor_id)[0]);
...
void WndChooseAccount::SetAccountAllowLoginStatus(int datagrid_row_index, bool row_checkstate)
{
	...
	for (auto it = Global::memory.broker_info_list.begin(); it != Global::memory.broker_info_list.end(); ++it)
	{
		if (it->broker_id == broker_id)
		{
			...
			for (auto it1 = it->invest_account_list.begin(); it1 != it->invest_account_list.end(); ++it1)
			{
				if (String::Contains(std::string(investor_id), it1->investor_id))
				{
					if (row_checkstate)
					{
						if (it->order_front.second == UNCONNECTABLE)
						{
							status_bar_choose_account.SetBlockText(0, L"for evaluation only at offline state");
						}
					...
					{
						int check_state = false;
						for (int i = 0; i < datagrid_account_list.GetRowCount(); ++i)
						{
