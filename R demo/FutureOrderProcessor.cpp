...
FutureOrderProcessor::FutureOrderProcessor(FutureOrderShooter* _order_shooter, BrokerInfo* _broker_info, InvestAccountSync* _invest_account, unsigned int _index)
{
...
void FutureOrderProcessor::OnFrontConnected()
{
	...
	case -1:
	{
		msg = "failed to log in " + broker_info->broker_name + " ctp order server. network connection failed";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);	
	}
		break;
	case -2:
	{
		msg = "failed to log in " + broker_info->broker_name + " ctp order server. unprocessed requests exceed limit";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
		break;
	case -3:
	{
		msg = "failed to log in " + broker_info->broker_name + " ctp order server. requests sent per second exceed limit";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
		break;
	default:
...
void FutureOrderProcessor::OnFrontDisconnected(int nReason)
{
	...
	case 0x1001:
	{
		msg = "failed to keep connection to " + broker_info->broker_name + " ctp order server. failed to read on network";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
	break;
	case 0x1002:
	{
		msg = "failed to keep connection to " + broker_info->broker_name + " ctp order server. failed to write on network";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
	break;
	case 0x2001:
	{
		msg = "failed to keep connection to " + broker_info->broker_name + " ctp order server. receive heartbeat timeout";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
	break;
	case 0x2002:
	{
		msg = "failed to keep connection to " + broker_info->broker_name + " ctp order server. send heartbeat failed";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
	break;
	case 0x2003:
	{
		msg = "failed to keep connection to " + broker_info->broker_name + " ctp order server. received errorneous packets";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
	break;
	default:
...
void FutureOrderProcessor::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	{
		msg = "failed to log in " + broker_info->broker_name + " ctp order server";
	...
	else if (pRspUserLogin)
	{
		...
		msg = "succeeded in logging in " + broker_info->broker_name + " ctp order server";
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
...
void FutureOrderProcessor::OnRspSettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (CTPShared::IsErrorRspInfo(pRspInfo))
	{
		msg = "failed to confirm settlement info of" + broker_info->broker_name + " " + invest_account->investor_id;
		CTPShared::FormatRspInfo(pRspInfo, &msg);
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
	else
...
void FutureOrderProcessor::OnRspQryTradingAccount(CThostFtdcTradingAccountField *pTradingAccount, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (CTPShared::IsErrorRspInfo(pRspInfo))
	{
	    msg = "failed to query account info of " + broker_info->broker_name + " " + invest_account->investor_id;
		CTPShared::FormatRspInfo(pRspInfo, &msg);
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
...
void FutureOrderProcessor::OnRspQrySettlementInfo(CThostFtdcSettlementInfoField *pSettlementInfo, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (CTPShared::IsErrorRspInfo(pRspInfo))
	{
		msg = "failed to query " + broker_info->broker_name + " " + invest_account->investor_id + " " + "settlement info";
		CTPShared::FormatRspInfo(pRspInfo, &msg);
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
...
void FutureOrderProcessor::OnRspQryInvestorPosition(CThostFtdcInvestorPositionField *pInvestorPosition, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (CTPShared::IsErrorRspInfo(pRspInfo))
	{
		msg = "failed to query position of" + broker_info->broker_name + " " + invest_account->investor_id;
		CTPShared::FormatRspInfo(pRspInfo, &msg);
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
...
void FutureOrderProcessor::OnRspQrySettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (CTPShared::IsErrorRspInfo(pRspInfo))
	{
		msg = "failed to query " + broker_info->broker_name + " " + invest_account->investor_id + " " + "settlement confirm";
		CTPShared::FormatRspInfo(pRspInfo, &msg);
		(*order_shooter->handler)(msg.c_str(), (size_t)index, NULL);
	}
...
#pragma endregion
				...
				{
					//5s后未成交自动撤单
					order_shooter->future_order_manager.RegisterOpenOrder(broker_info, invest_account, &oc);
				}
			}
		}
	}
