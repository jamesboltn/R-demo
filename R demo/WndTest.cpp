...
void WndTest::InitComponents()
{
	...
	tree_view = TreeView(7, 0, 61, 115, DIALOG_UNIT);
	...
	AddHandler(TVN_SELCHANGED, static_cast<MessageHandler>(&WndTest::OnTreeViewSelectionChange));
...
long WndTest::OnTreeViewSelectionChange(unsigned int wParam, long lParam)
{
	char buffer[50];	
	tree_view.GetChainedTextOfSelectedItem(buffer, ARRAYCOUNT(buffer));
	MessageBoxA(buffer);
	return 0;
}
