...
public:
	...
	void Create(const char* table_name);//functions the same as member CreateTable
	void AddFieldNotNull(const char* col_name, DataType data_type);
	void AddField(const char* col_name, DataType data_type);
	SQLite &Field(const char* col_name, DataType data_type);//functions the same as member AddField
