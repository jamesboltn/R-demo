...
void SQLite::AddField(const char* col_name, DataType data_type)
{
...
SQLite &SQLite::Field(const char* col_name, DataType data_type)
{
	field.col_name = col_name;
	field.data_type = data_type;
	field.null_flag = _NULL;
...
void SQLite::Create(const char* table_name)
{
	if (IsConnected() && strcmp(table_name, "") != 0 && !fields.empty())
	{
		statement = "CREATE TABLE IF NOT EXISTS '" + std::string(table_name) + "' ('";
		for (auto it = fields.begin(); it != fields.end(); ++it)
		{
