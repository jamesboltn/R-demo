...
void StockPositionModel::Retrieve(SQLite* db, const char* table_name, std::multiset<PositionDetailSync, CompareByValue>* position_list, AccountSpecificMsgHandler handler, unsigned int index)
{
	...
	if (database_error_message != "ok")
	{
		if (handler != NULL)
		{
			(*handler)(database_error_message, (size_t)index);
		}
	}
	else
	{
...
void StockPositionModel::Update(SQLite* db, const char* table_name, std::multiset<PositionDetailSync, CompareByValue>* position_list, AccountSpecificMsgHandler handler, unsigned int index)
{
